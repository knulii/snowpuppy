...
port 9050 and only allows .onion nodes to connect:
	...
	./snowpuppycoind -onion=127.0.0.1:9050 -onlynet=tor -listen=0 -addnode=ssapp53tmftyjmjb.onion
...
versions of Tor see [Section 4](#4-automatically-listen-on-tor).*
	...
	HiddenServiceDir /var/lib/tor/snowpuppycoincore-service/
...
In a typical situation, where you're only reachable via Tor, this should suffice:
	...
	./snowpuppycoind -proxy=127.0.0.1:9050 -externalip=ssapp53tmftyjmjb.onion -listen

(obviously, replace the Onion address with your own). It should be noted that you still
listen on all devices and another node could establish a clearnet connection, when knowing
your address. To mitigate this, additionally bind the address of your Tor proxy:

	./snowpuppycoind ... -bind=127.0.0.1

If you don't care too much about hiding your node, and want to be reachable on IPv4
as well, use `discover` instead:

	./snowpuppycoind ... -discover
...
configured. For cookie authentication the user running snowpuppycoind must have write access
to the `CookieAuthFile` specified in Tor configuration. In some cases this is
preconfigured and the creation of a hidden service is automatic. If permission problems
are seen with `-debug=tor` they can be resolved by adding both the user running Tor and
the user running snowpuppycoind to the same group and setting permissions appropriately. On
Debian-based systems the user running snowpuppycoind can be added to the debian-tor group,
which has the appropriate permissions. Before starting snowpuppycoind you will need to re-login
