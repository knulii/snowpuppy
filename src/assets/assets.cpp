...
static const std::regex spupc_names("^SPUPC$|^SNOWPUPPYCOIN$|^wSPUPC$|^WSPUPC$|^SPUPCcoin$|^SPUPCCOIN$");


bool IsAssetNameValid(std::string name, bool isRoot) {
    if (name.length() < 3 || name.length() > 128) return false;
    if (isRoot)
        return std::regex_match(name, name_root_characters) && !std::regex_match(name, spupc_names);
    else
        return std::regex_match(name, name_sub_characters) && !std::regex_match(name, spupc_names); 
}
