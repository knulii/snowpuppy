...
static bool AppInit(int argc, char *argv[]) {
    ...
    // Parameters
    //
    // If Qt is used, parameters/snowpuppycoin.conf are parsed in qt/snowpuppycoin.cpp's main()
    SetupServerArgs();
    std::string error;
    ...
    if (HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (gArgs.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage: snowpuppycoind [options]           Start "
            PACKAGE_NAME
            " Daemon\n";
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(strprintf(
                        "Error: Command line contains unexpected token '%s', see snowpuppycoind -h for a list of options.\n",
                        argv[i]));
            }
        }

        // -server defaults to true for snowpuppycoind but not for the GUI so do this here
...
int main(int argc, char *argv[]) {
    ...
    // Connect snowpuppycoind signal handlers
    noui_connect();
