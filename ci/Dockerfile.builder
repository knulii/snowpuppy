...
# snowpuppycoin_hash
RUN git clone https://github.com/raptor3um/snowpuppycoin_hash
RUN cd snowpuppycoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} snowpuppycoin
RUN useradd -u ${USER_ID} -g snowpuppycoin -s /bin/bash -m -d /snowpuppycoin snowpuppycoin
...
RUN mkdir /snowpuppycoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /snowpuppycoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /snowpuppycoin-src
