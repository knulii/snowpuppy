...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/SnowpuppycoinParams"
else
    PARAMS_DIR="$HOME/.snowpuppycoin-params"
fi
...
DOWNLOAD_URL="https://github.com/Raptor3um/snowpuppycoin/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with SPUPC_DISABLE_SOMETHING=1
SPUPC_DISABLE_WGET="${SPUPC_DISABLE_WGET:-}"
#SPUPC_DISABLE_IPFS="${SPUPC_DISABLE_IPFS:-}"
SPUPC_DISABLE_CURL="${SPUPC_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$SPUPC_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$SPUPC_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$SPUPC_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Snowpuppycoin parameters!
...
main() {
    ...
    cat <<EOF
SPUPC - fetch-params.sh

This script will fetch the Snowpuppycoin SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Snowpuppycoin SNARK parameters. Note that it is
